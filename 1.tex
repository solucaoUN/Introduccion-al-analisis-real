...
\item[✎] \textbf{Verdadero: } \begin{proof}
...
\begin{itemize}
    ...
    \item[✎]\textbf{Verdadero: } \begin{proof}
        Como en las pruebas anteriores note que si $r\leq1$ entonces para todo $x\in \mathbb{R}$, $B(x,r)\setminus\{x\}=$\O y por tanto $B(x,r)\setminus\{x\}\cap \mathbb{R}=$\O, luego $\mathbb{R}^{\prime}=$\O
    \end{proof}
...
\end{itemize}
 ...
 \quad \quad los siguientes ejercicios fueron propuestos en 2022-2 en un parcial de Análisis, el propósito de esta sección es que sirva para que el estudiante pueda hacerse una idea del modelo de preguntas que se suelen hacer y esté preparado
...
entonces $f_n \in B\left(f_0, \frac{3}{2}\right)$ para todo $n \in \mathbb{Z}^{+}$.
 ...
 \begin{proof}
    ...
    \begin{align*}
        d(f_n,f_0)&=\sup_{x \in I}\left|\dfrac{x^2sen(2\pi nx)}{2\pi n}-x^2\right|\\
        &=\sup_{x \in I}\left|x^2\left(\dfrac{sen(2\pi nx)}{2\pi n}-1\right)\right|\\
        &=\sup_{x \in I}\left|x^2\left(\dfrac{sen(kx)}{k}-1\right)\right| \quad (k=2\pi n)\\
    \end{align*}
    
Tenemos que $k>1$ ya que $n \in \mathbb{Z}^+$, luego $-1\leq \dfrac{sen{kx}}{k}\leq 1$, por tanto:
...
\end{proof}  
   ...
   \quad \quad \textbf{Comentario:} cuando decimos métrica sobre $X^2$ queremos decir que la métrica va del producto cartesiano $X \text{ x } X$ en $\bb{R}$, no confundir con $X^2 \text{ x } X^2$ 
