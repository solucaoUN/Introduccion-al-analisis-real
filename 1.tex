...
% Text Node
\draw (468,83) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,opacity=1 ] [align=left] {$\displaystyle A$};
% Text Node
\draw (76,56) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,opacity=1 ] [align=left] {$\displaystyle \overline{A+B}$};
% Text Node
...
\begin{itemize}[label={✎},leftmargin=*]
    ...
    \begin{proof}
        ...
        $$d_{\infty}(x_n,L)\leq \frac{\epsilon}{n}$$
...
\item Sea $(X,d)$ un espacio métrico, si $S\supseteq X$, entonces $(S^{\prime})^{\prime}=S^{\prime}$.\\
...
$$\dfrac{d(x_n,L)}{1+d(x_n,L)}<d(x_n,L)<\epsilon$$
...
\begin{align*}
 ...
 &\leq d(y,u)-(d(x,y)+d(y,v))\\
 &\leq d(y,u)-(d(x,y)+d(y,u)+d(u,v))\\
 &=-d(x,y)-d(u,v)\\
 &\leq d(x,y)+d(u,v)  
\end{align*}

Ahora si $|d(x,v)-d(y,u)|>0$, entonces:
